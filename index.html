...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  #view-salarie > .card:first-child{ margin-top: 12px; }
  ...
  .calc-status.ok{ color: var(--teal); }
  .calc-status.err{ color: var(--danger); }
  .calc-status .route{ display:block; color: var(--ink-soft); margin-top:2px; }
  ...
  .ac-item{ padding:9px 12px; font-size:13px; cursor:pointer; border-bottom:1px solid var(--line); }
  .ac-item:last-child{ border-bottom:none; }
  .ac-item.active, .ac-item:hover{ background: var(--teal-wash); }
  .ac-item small{ display:block; color: var(--ink-soft); font-size:11px; }
  input.addr-ok{ border-color: var(--teal) !important; }
  ...
  /* ===== Vue direction ===== */
  #view-direction .status-row{ display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; flex-wrap:wrap; gap:8px; }
  #view-direction .live-dot{ display:inline-flex; align-items:center; gap:6px; font-size:11px; color: var(--ink-soft); }
  #view-direction .live-dot .dot{ width:7px; height:7px; border-radius:50%; background:var(--teal); animation: dirpulse 1.6s infinite; }
  @keyframes dirpulse{ 0%,100%{opacity:1;} 50%{opacity:0.3;} }
  #view-direction table.dir-table{ width:100%; border-collapse:collapse; font-size:13px; }
  #view-direction table.dir-table th, #view-direction table.dir-table td{ border:1px solid var(--line); padding:9px 10px; text-align:left; }
  #view-direction table.dir-table thead th{
  ...
  #view-direction td.num{ font-family: var(--font-mono); font-weight:600; white-space:nowrap; }
  #view-direction td.km{ color: var(--teal); }
  #view-direction td.forfait{ color: var(--coral); }
  #view-direction tr.total td{ background: var(--navy); color:white; font-weight:700; }
  #view-direction tr.salarie-row{ cursor:pointer; }
  #view-direction tr.salarie-row:hover td{ background: var(--teal-wash); }
  #view-direction tr.salarie-row td:first-child{ display:flex; align-items:center; gap:6px; }
  #view-direction tr.salarie-row .chevron{ display:inline-block; transition: transform 0.15s; font-size:11px; color: var(--ink-soft); }
  #view-direction tr.salarie-row.open .chevron{ transform: rotate(90deg); }
  #view-direction tr.detail-row td{ padding:0; border-top:none; }
  #view-direction .detail-wrap{ padding:10px 14px 14px; background:#FAFBFF; }
  #view-direction table.detail-table{ width:100%; border-collapse:collapse; font-size:12px; margin-top:6px; }
  #view-direction table.detail-table th, #view-direction table.detail-table td{ border:1px solid var(--line); padding:6px 8px; text-align:left; }
  #view-direction table.detail-table thead th{ background: var(--navy-wash); color:var(--navy); font-size:10.5px; text-transform:uppercase; }
  #view-direction table.detail-table td.cell-total{ background: var(--teal-wash); font-family: var(--font-mono); font-weight:700; color:var(--navy); }
  #view-direction table.detail-table td.cell-forfait{ background: var(--coral-wash); font-family: var(--font-mono); font-weight:700; color:var(--coral); }
  #view-direction .detail-loading{ font-size:12px; color: var(--ink-soft); padding:8px 0; }
  #view-direction .empty-dir{ text-align:center; padding:24px; color: var(--ink-soft); font-size:13.5px; }
  #view-direction .error-box{ background: var(--coral-wash); color:#8a3220; border-radius:10px; padding:10px 14px; font-size:13px; margin-bottom:12px; }
  #view-direction .btn-login{ background: var(--teal); color:white; width:100%; margin-top:12px; }
  #view-direction .btn-login:hover{ background: var(--navy); }
  #view-direction .table-wrap{ overflow-x:auto; }

  @media print{
    .hero, .view-tabs, .toolbar, .swap-btn, .leg-del, .no-print, .btn-primary, .checkbox-row, .total-bar{ display:none !important; }
    ...
    <div class="card">
      ...
      <div class="leg-block">
        ...
        <div class="field-row" style="margin-top:10px; align-items:flex-end;">
          ...
          <button class="tab-btn" id="btn-calc-aller" type="button">📍 Calculer</button>
        </div>
      </div>
      ...
      <div class="leg-block" id="retour-block">
        ...
        <div class="field-row" style="margin-top:10px; align-items:flex-end;">
          ...
          <button class="tab-btn" id="btn-calc-retour" type="button">📍 Calculer</button>
        </div>
      </div>
...
<div class="total-bar no-print" id="total-bar">
  <div class="tb-group">
...
(function(){
  ...
  const els = {
    ...
    btnCalcAller: $('btn-calc-aller'), btnCalcRetour: $('btn-calc-retour'),
  ...
  async function loadDomicile(){
    ...
    try{
      ...
      // setAddr remet aussi à zéro les coordonnées du salarié précédent
      setAddr(els.domicile, (data && data.domicile) || '');
  ...
  async function addTrip(){
    ...
    try{
      ...
      els.fKm.value = '';
      setAddr(els.fArrivee, '');
      els.fRetourKm.value = '';
      setAddr(els.fRetourArrivee, '');
      setAddr(els.fRetourDepart, '');
      els.allerCalcStatus.textContent = ''; els.retourCalcStatus.textContent = '';
      els.fKm.focus();
    }catch(e){
  ...
  // --- Adresses : autocomplétion BAN + coordonnées mémorisées sur le champ ---
  // Quand le salarié choisit une suggestion, on garde les coordonnées exactes
  // (data-lon / data-lat) : le calcul ne dépend plus d'une interprétation du texte.
  function clearCoords(input){
    delete input.dataset.lon; delete input.dataset.lat;
    input.classList.remove('addr-ok');
  }
  function setAddr(input, label, lon, lat){
    input.value = label || '';
    if(lon != null && lat != null){
      input.dataset.lon = lon; input.dataset.lat = lat;
      input.classList.add('addr-ok');
    } else {
      clearCoords(input);
    }
  }
  function copyAddr(from, to){
    setAddr(to, from.value, from.dataset.lon, from.dataset.lat);
  }
  ...
  function attachAutocomplete(input){
    ...
    let items = [], active = -1, timer = null, reqId = 0;

    function close(){ list.classList.remove('open'); list.innerHTML = ''; items = []; active = -1; }
    ...
    function choose(i){
      ...
      setAddr(input, it.label, it.lon, it.lat);
      close();
    ...
    async function search(){
      ...
      try{
        ...
        const data = await res.json();
        if(my !== reqId || document.activeElement !== input) return;
        items = Array.isArray(data) ? data : [];
        if(items.length === 0){ close(); return; }
        list.innerHTML = items.map((it, i) =>
          `<div class="ac-item" data-i="${i}">${escapeHtml(it.label)}${it.type === 'street' ? '<small>rue entière (numéro non référencé)</small>' : ''}</div>`
        ).join('');
        list.classList.add('open');
        active = -1;
      }catch(e){ close(); }
    }
    // mousedown + preventDefault : le champ garde le focus, donc pas de
    // "change" prématuré avec un texte à moitié tapé.
  ...
  // --- Calcul automatique de la distance (BAN + itinéraire IGN) ---
  const calcSeq = new WeakMap();
  async function autoCalcKm(departEl, arriveeEl, kmEl, statusEl){
    const depart = departEl.value.trim(), arrivee = arriveeEl.value.trim();
    if(!depart || !arrivee) return;
    const my = (calcSeq.get(kmEl) || 0) + 1;
    calcSeq.set(kmEl, my);
    const setStatus = (html, cls) => { if(statusEl){ statusEl.innerHTML = html; statusEl.className = 'calc-status ' + (cls || ''); } };
    setStatus('Calcul de la distance...');
    const params = new URLSearchParams({ depart, arrivee });
    if(departEl.dataset.lon){ params.set('dlon', departEl.dataset.lon); params.set('dlat', departEl.dataset.lat); }
    if(arriveeEl.dataset.lon){ params.set('alon', arriveeEl.dataset.lon); params.set('alat', arriveeEl.dataset.lat); }
    try{
      const res = await fetch('/api/distance?' + params.toString());
      const data = await res.json();
      if(calcSeq.get(kmEl) !== my) return; // une saisie plus récente a pris le relais
      if(res.ok && typeof data.km === 'number'){
        kmEl.value = String(data.km);
        const warn = (data.warnings || []).length ? `<span class="route">⚠ ${escapeHtml(data.warnings.join(' ; '))}</span>` : '';
        setStatus(
          `Distance calculée : ${String(data.km).replace('.', ',')} km` +
          `<span class="route">${escapeHtml(data.depart)} → ${escapeHtml(data.arrivee)}</span>` + warn,
          'ok'
        );
      } else {
        kmEl.value = '';
        setStatus(escapeHtml(data.error || 'Distance non trouvée') + ' — sinon, renseignez le km manuellement', 'err');
      }
    }catch(e){
      if(calcSeq.get(kmEl) !== my) return;
      setStatus('Calcul indisponible, merci de renseigner le km manuellement', 'err');
    }
  }

  function maybeAutoCalc(departEl, arriveeEl, kmEl, statusEl){
    autoCalcKm(departEl, arriveeEl, kmEl, statusEl);
  }

  function copyRetourFromAller(){
    copyAddr(els.fArrivee, els.fRetourDepart);
    copyAddr(els.fDepart, els.fRetourArrivee);
  ...
  function swapAddresses(){
    const tmp = { value: els.fDepart.value, dataset: { ...els.fDepart.dataset } };
    copyAddr(els.fArrivee, els.fDepart);
    copyAddr(tmp, els.fArrivee);
    maybeAutoCalc(els.fDepart, els.fArrivee, els.fKm, els.allerCalcStatus);
  }

  [els.domicile, els.fDepart, els.fArrivee, els.fRetourDepart, els.fRetourArrivee].forEach(attachAutocomplete);
  ...
  els.btnCalcAller.addEventListener('click', () => maybeAutoCalc(els.fDepart, els.fArrivee, els.fKm, els.allerCalcStatus));
  els.btnCalcRetour.addEventListener('click', () => maybeAutoCalc(els.fRetourDepart, els.fRetourArrivee, els.fRetourKm, els.retourCalcStatus));
  ...
  els.salarie.addEventListener('change', async () => {
    ...
    await loadDomicile();
    if(!els.fDepart.value) copyAddr(els.domicile, els.fDepart);
    await loadEntries();
  });
